...
class TestXciteParserp(unittest.TestCase):
    ...
    def test_parse_2(self):
        ...
        self.assertEqual(first_cluster,
                         CitationCluster([CitationInfo(citekey=item1, author_only=True)]))
        self.assertEqual(second_cluster,
                         CitationCluster([CitationInfo(citekey=item1, suppress_author=True, locator="p. 30")]))

    # @item1 [p. 30, with suffix]
    # XXX - is this parse right?
    def test_parse_3(self):
        item1 = self.mk_citekey()
        [first_cluster, second_cluster] = self.parse("@%s [p. 30, with suffix]"%(item1))
        self.assertEqual(first_cluster,
                         CitationCluster([CitationInfo(citekey=item1, author_only=True)]))
        self.assertEqual(second_cluster,
    ...
    def test_parse_4(self):
        ...
        [first_cluster, second_cluster] = self.parse("@%s [-@%s p. 30; see also @%s]"%(item1, item2, item3))
        self.assertEqual(first_cluster,
                         CitationCluster([CitationInfo(citekey=item1, author_only=True)]))
        self.assertEqual(second_cluster,
                         CitationCluster([CitationInfo(citekey=item1,
                                                       suppress_author=True),
                                          CitationInfo(citekey=item2,
                                                       suppress_author=True,
                                                       locator="p. 30"),
    ...
    def test_parse_5(self):
        item1 = self.mk_citekey()
        item3 = self.mk_citekey()
        [first_cluster, second_cluster] = self.parse("[see @%s p. 34-35; also @%s chap. 3]"%(item1, item3))
    ...
    def test_parse_7(self):
        item1 = self.mk_citekey()
        [first_cluster, second_cluster] = self.parse("[@%s pp. 33, 35-37 and nowhere else]"%(item1))
        self.assertEqual(first_cluster, None)
        self.assertEqual(second_cluster,
                         CitationCluster([CitationInfo(citekey=item1,
                                                       locator="pp. 33, 35-37",
    ...
    def test_parse_10(self):
        ...
        self.assertEqual(first_cluster, None)
        self.assertEqual(second_cluster,
                         CitationCluster([CitationInfo(citekey=item3)]))
    ...
    def test_parse_11(self):
        ...
        item2 = self.mk_citekey()
        item3 = self.mk_citekey()
        [first_cluster, second_cluster] = self.parse("[see @%s chap. 3; @%s; @%s]"%(item2, item3, item1))
        self.assertEqual(first_cluster, None)
        self.assertEqual(second_cluster,
                         CitationCluster([CitationInfo(citekey=item2,
                                                       prefix="see",
                                                       locator="chap. 3"),
                                          CitationInfo(citekey=item3),
