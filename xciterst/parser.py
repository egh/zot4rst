...
class CiteParser(object):
    ...
    def parse(self, what):
        ...
        emText.setParseAction(lambda s, l, t: "<i>%s</i>" % (" ".join(t[1:-1])))
        strongText = "**" + OneOrMore(greedyToken) + "**"
        strongText.setParseAction(lambda s, l, t: "<b>%s</b>" % (" ".join(t[1:-1])))

        text = strongText | emText | greedyToken

        locator = (Optional(",") + OneOrMore(wordWithDigits)) ^ (
            Optional(",") + Optional(greedyToken) + OneOrMore(wordWithDigits)
        )

        def locator_parse_action(s, l, t):
            raw = " ".join(t)
            # strip leading comma
            return CiteParser.Locator(re.sub(r"^,\s+", "", raw))
        ...
        topCite = (
            bracketedCite
            ^ shortCite + shortCiteExtra
            ^ shortCite + bracketedCite
            ^ shortCite
        )
